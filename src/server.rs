...
pub async fn accept_loop(addr: impl ToSocketAddrs) -> Result<()> {
    ...
    while let Some(stream) = incoming.next().await {
        ...
        let handle = task::spawn(connection_handler(stream)).await;
        let _conn = handle.unwrap();
    }
...
async fn connection_handler(stream: TcpStream) -> Result<()> {
    ...
    let (p, query) = http_base::parse_url_query_string(path);
    ...
    let response = http_base::HTTPResponse {
        ...
        body: parsed_request.path
    };*/

    let response = http_handler::req(parsed_request);
